...
async function findDuplicateStockOut(tenant, line, itemRegistrationId) {
  const candidates = await prisma.stockOutRequest.findMany({
    where: {
      HotelName: tenant,
      itemRegistrationId,
...
/**
 * Resolve an existing registration. Same name may appear more than once —
 * prefer a row with enough on-hand qty (most recent first). Never reject solely
 * because another row already used the same name.
 */
async function findRegistrationForStockOut(tenant, line) {
  ...
  const matches = await prisma.itemRegistration.findMany({
    where: { HotelName: tenant, name: line.itemName },
    orderBy: { id: "desc" },
  });
  if (!matches.length) return null;

  const withStock = matches.find(
    (m) => Number(m.amount) >= Number(line.amount),
  );
...
async function importStockOutRequests(tenant, aliases, rows) {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    try {
      ...
      const item = await findRegistrationForStockOut(tenant, line);

      if (item) {
        const dup = await findDuplicateStockOut(tenant, line, item.id);
        if (dup) {
          throw new Error(
            `Stock movement already exists (id ${dup.id}) with the same required fields`,
          );
        }
