 ...
 * Same name may appear more than once — prefer newest. Matching is
 * case-insensitive / whitespace-normalized.
 */
function pickRegistrationFromCache(line, byId, byName) {
  ...
  const matches = byName.get(normalizeItemNameKey(line.itemName)) || [];
...
/**
 * Copy pricing + supplier (and other receipt fields) from the matched
 * ItemRegistration onto the stock-movement receipt. Does not change
 * registration on-hand quantity. paidAmount comes from the registration
 * as stored (credit/partial allowed for other tenants).
 */
function receiptFieldsFromRegistration(item, moveAmt, actionDate, reqRow, actorName) {
  return {
    name: String(item.name ?? "").trim(),
    imageUrl: String(item.imageUrl ?? "").trim(),
    category: String(item.category ?? "").trim() || "Others",
    amount: moveAmt,
    measuredBy: String(item.measuredBy ?? "").trim() || "Piece",
...
/**
 * Apex Excel seed: record the approved movement for receipts / request status
 * WITHOUT changing ItemRegistration.on-hand — the registration Excel already
 * carries the final inventory quantity. When the item name matches a
 * registration, unit price, paid amount, and supplier fields are taken from it.
 */
async function recordStockOutReceiptOnly(tx, reqRow, item, actorName) {
  ...
  await tx.itemStatus.create({
    data: receiptFieldsFromRegistration(
      item,
      moveAmt,
      actionDate,
      reqRow,
...
async function importStockOutRequests(tenant, aliases, rows) {
  ...
  // Load tenant registrations so name matching can be case-insensitive.
  const regs = await prisma.itemRegistration.findMany({
  ...
  for (const r of regs) {
    const key = normalizeItemNameKey(r.name);
    if (!key) continue;
  ...
  for (const { rowNum, line } of parsed) {
    try {
      ...
      const item = pickRegistrationFromCache(line, byId, byName);
      ...
      if (item) {
        ...
        await prisma.$transaction(async (tx) => {
          ...
          // Do not deduct ItemRegistration.amount — seed Excel already has final qty.
          await recordStockOutReceiptOnly(tx, reqRow, item, APEX_SEED_ACTOR);
