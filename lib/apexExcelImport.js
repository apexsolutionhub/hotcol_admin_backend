...
function itemRegistrationSeedData(line, tenant, voucherNumber, now) {
  const mapped = mapItemRegistrationLine(line);
  return {
    ...mapped,
    HotelName: tenant,
    voucherNumber,
...
async function insertItemRegistrationRows(
  rows,
  tenant,
  voucherNumber,
  now,
  ...
  for (let start = 0; start < rows.length; start += CHUNK_SIZE) {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        await tx.itemRegistration.createMany({
          data: slice.map((row) =>
            itemRegistrationSeedData(row, tenant, voucherNumber, now),
          ),
        });
      }, BATCH_TX_OPTS);
      ...
      for (let i = 0; i < slice.length; i += 1) {
        const rowNum = rowOffset + start + i + 1;
        try {
          await prisma.itemRegistration.create({
            data: itemRegistrationSeedData(
              slice[i],
              tenant,
              voucherNumber,
              now,
...
async function importItemRegistrations(tenant, aliases, rows) {
  ...
  const errors = [];
  const importedCount = await insertItemRegistrationRows(
    rows,
    tenant,
    voucherNumber,
    now,
