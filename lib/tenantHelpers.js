...
import { salesAgentNameFromAccount } from "./salesAgents.js";
...
export async function ensureTenantAccount(tinNumber, owner) {
  ...
  const defaultMode = cafeModuleSelected(modules)
    ? parseCafeOrderMode(owner?.cafeOrderMode)
    : parseCafeOrderMode(owner?.cafeOrderMode);
  let account = await prisma.tenant_account.findUnique({ where: { tinNumber: tin } });
  if (account) {
    if (
      cafeModuleSelected(parseModulesJson(account.modules ?? modules)) &&
      String(account.cafeOrderMode || "").trim() === ""
    ) {
      account = await prisma.tenant_account.update({
        where: { tinNumber: tin },
        data: { cafeOrderMode: "digital" },
      });
    }
    return account;
  }
...
export function buildTenantListItem(owner, account, subscriptionStatus, unreadFeedback = 0) {
  ...
  return {
    ...
    unreadFeedback,
    cafeOrderMode: cafeOrderModeForModules(modules, account?.cafeOrderMode),
    salesAgentId: account?.salesAgentId ?? account?.salesAgent?.id ?? null,
